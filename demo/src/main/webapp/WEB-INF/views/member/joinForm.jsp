...
//** 실습
// 1) 전역변수 정의
let iCheck,pCheck,p2Check,nCheck,aCheck,poCheck,bCheck = false;
...
onload = function(){
// => window.onload 에서 window. 는 생략가능
// => body 의 Tag 들을 인식가능한 상태에서 실행되도록 하기위함.
	...
	document.getElementById("id").addEventListener('keydown',(e) => {
		if(e.which==13){
			//=> form Tag 내에서는 enter 사용 시 submit 발생
			// 그러므로 e 이벤트의 제거가 필요함
			e.preventDefault();
...
//3) submit 실행 판단 & 실행
// => 모든 항목에 대한 무결성 확인
// => 모두 오류가 없으면 submit 진행하지 않고 수정유도

function inCheck(){
	if(!iCheck){
		document.getElementById("iMessage").innerHTML = 'id는 필수입력입니다. 다시 확인하세요~';
	}
	if(!pCheck){
		document.getElementById("pMessage").innerHTML = '비밀번호는 필수입력입니다. 다시 확인하세요~';
	}
	if(!p2Check){
		document.getElementById("p2Message").innerHTML = '비밀번호 확인은 필수입력입니다. 다시 확인하세요~';
	}
	if(!nCheck){
		document.getElementById("nMessage").innerHTML = '이름은 필수입력입니다. 다시 확인하세요~';
	}
	if(!aCheck){
		document.getElementById("aMessage").innerHTML = '나이는 필수입력입니다. 다시 확인하세요~';
	}
	if(!poCheck){
		document.getElementById("poMessage").innerHTML = '점수는 필수입력입니다. 다시 확인하세요~';
	}
	if(!bCheck){
		document.getElementById("bMessage").innerHTML = '생일은 필수입력입니다. 다시 확인하세요~';
	}
	if(iCheck && pCheck && p2Check && nCheck && aCheck && poCheck && bCheck){
		if(confirm("정말 가입하시겠습니까?(Yes:확인 / No:취소)")){
			return true;
		}else{
		...
		<table>
			<tr height="40">
				...
				<td><input type="text" name="id" id="id"
					placeholder="영문과 숫자로 4~10글자" size="20">
			...
			<tr height="40">
				...
				<td><input type="password" name="password" id="password"
					placeholder="특수문자 필수" size="20"> <br> <span
					id="pMessage" class="eMessage"></span></td>
			</tr>
			<tr height="40">
				<td bgcolor="#bcdeff"><label for="password2">PW check</label></td>
				<td><input type="password" id="password2"
					placeholder="PW 재입력 확인" size="20"> <br> <span
			...
			<tr height="40">
				...
				<td><select name="jno" id="jno">
						<c:forEach var="j" items="${requestScope.joList}">
							<option value="${j.jno}">${j.jno}조:${j.jname}</option>
						</c:forEach>
				</select></td>
			</tr>
			<!-- 	<tr height="40">
			...
			<tr height="40">
				...
				<td><input type="text" name="point" id="point"
					placeholder="실수 입력" size="20"><br> <span
					id="poMessage" class="eMessage"></span></td>
			</tr>
			<tr height="40">
