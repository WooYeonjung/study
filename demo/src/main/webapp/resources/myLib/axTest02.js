...
// 2) Join처리: axiJoin
// => multipart 타입 포함
// => JS의 내장객체 FormData 사용 (JSON이 아닌 JS 객체)
//        -> 요청_headers "Content-Type" 변경
function axiJoin() {
	let url = "/rest/rsjoin";

	// 2.1) Data 전송준비
	let formData = new FormData(document.getElementById("myform")); //file 값까지 받아줌

	//2.2) Axios 요청 처리
	// => axiow.post(url,data[, config])    => , config : 추가 형식이 필요하다면 추가
	axios.post(url, formData,
		...
		if (err.response.status == '502') alert(`입력 오류! 다시 하세요.`)
			else alert (`System 오류, 잠시 후 다시하세요. status => ${err.message}`)
	});
	document.getElementById("resultArea2").innerHTML = '';
}//axijoin
